  ...
  import 'package:hanouty/Core/network/apiconastant.dart'; // for ApiConstants
  ...
  class AuctionBidScreen extends StatefulWidget {
    ...
    final String? cropId;
    final String? endTime; // Pass endTime as ISO string, e.g. '2025-06-06T10:00:00.000+00:00'
  ...
  class _AuctionBidScreenState extends State<AuctionBidScreen> {
    ...
    Timer? _timer;
    Duration? _timeLeft;
    double? _startingPrice; // Add starting price field
    ...
    void initState() {
      ...
      socketClient.socket.on('joinedAuction', (data) {
        setState(() {
          ...
          // Set timer if endTime is available in response
          if (data['auction']?['endTime'] != null) {
            _setEndTime(data['auction']['endTime']);
          }
        });
        ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('Joined auction room!'))
        );
      });

      socketClient.socket.on('userJoined', (data) {
        setState(() {
          bids.add({
            'type': 'join',
      ...
      socketClient.socket.on('marketSelectionRequired', (data) async {
        // data: { auctionId, cropId, markets: [ ... ], totalPrice }
        print("Received marketSelectionRequired: $data");
        String? selectedMarket = await showDialog<String>(
          context: context,
          barrierDismissible: false,
          builder: (ctx) {
            print("Market selection dialog data: $data");
            return AlertDialog(
              ...
              content: Column(
                ...
                children: (data['markets'] as List)
                  ...
                  title: Text(marketId), // You can show market name if you have it
        ...
        if (selectedMarket != null) {
          // Emit the selected market to the backend
      ...
      // Listen for orderCreated and orderError for feedback
    ...
    void _initTimer() {
      // Priority: use widget.endTime, fallback to nothing (will update on socket event)
    ...
    void dispose() {
      ...
      socketClient.socket.off('userJoined');
    ...
    num get _highestBid {
      final bidAmounts = bids
          .where((b) => b['type'] != 'join' && b['bidAmount'] != null)
